<div align="center">
  
# 👋 Привет! Я Иван Подымов

### Software Engineer | Full-Stack Developer

*Создаю современные веб-приложения с чистым кодом и элегантной архитектурой*

---

[![Telegram](https://img.shields.io/badge/Telegram-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white)](https://t.me/ipodymov)
[![VK](https://img.shields.io/badge/VK-0077FF?style=for-the-badge&logo=vk&logoColor=white)](https://vk.com/soulgrok)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:podymovv55@gmail.com)

</div>

---

## 🚀 О себе

Я **инженер-программист** с опытом разработки полнофункциональных веб-приложений. Специализируюсь на создании масштабируемых backend-систем и интуитивных пользовательских интерфейсов.

```typescript
const ivan = {
  role: "Full-Stack Developer",
  code: ["Python", "TypeScript", "JavaScript"],
  focus: ["API Design", "Clean Architecture", "Performance"],
  passion: "Building robust and elegant solutions"
};
```

### 💡 Мои приоритеты

- 🎯 **API-First Development** — проектирование надёжных и производительных API
- ⚡ **Modern Frontend** — создание отзывчивых SPA с современными фреймворками
- 🐳 **DevOps & Automation** — контейнеризация и автоматизация процессов разработки
- 📐 **Clean Code** — читаемый, поддерживаемый и хорошо протестированный код

---

## 🛠️ Технологический стек

### Backend
![NestJS](https://img.shields.io/badge/NestJS-E0234E?style=for-the-badge&logo=nestjs&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white)

### Frontend
![Vue.js](https://img.shields.io/badge/Vue.js-4FC08D?style=for-the-badge&logo=vuedotjs&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![Vuetify](https://img.shields.io/badge/Vuetify-1867C0?style=for-the-badge&logo=vuetify&logoColor=white)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white)

### Базы данных & ORM
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)
![TypeORM](https://img.shields.io/badge/TypeORM-FE0803?style=for-the-badge&logo=typeorm&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white)

### DevOps & Tools
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![REST API](https://img.shields.io/badge/REST_API-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![Swagger](https://img.shields.io/badge/Swagger-85EA2D?style=for-the-badge&logo=swagger&logoColor=black)
![JWT](https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=jsonwebtokens&logoColor=white)

---

## 💼 Избранные проекты

### 🎓 PD Projects — Платформа для учебных проектов
Полнофункциональная система управления проектами для студентов и школьников с модерацией и управлением участниками.

**Backend** — NestJS, TypeORM, PostgreSQL, JWT, Cache Manager
- 🔗 [pd-projects-backend](https://github.com/IPodymov/pd-projects-backend)
- ✨ Авторизация, модерация проектов, история изменений, кеширование
- 🚀 REST API с TypeORM и PostgreSQL

**Frontend** — Vue 3, TypeScript, Vite, Pinia, Vuetify 3
- 🔗 [pd-projects-frontend](https://github.com/IPodymov/pd-projects-frontend)
- ✨ SPA с управлением состоянием, роли пользователей, модерация
- 📱 Современный интерфейс с Vuetify 3

**ML Recommendation System** — Python, Machine Learning
- 🔗 [pd-projects-recomendation-system](https://github.com/IPodymov/pd-projects-recomendation-system)
- ✨ Рекомендательная система для проектов

---

### 📸 Fakegram — Instagram Clone
Полнофункциональный клон Instagram с постами, историями, чатами и уведомлениями.

**Backend** — NestJS, TypeORM, PostgreSQL, JWT
- 🔗 [fakegram-backend](https://github.com/IPodymov/fakegram-backend)
- ✨ Посты, истории, комментарии, лайки, подписки, чаты, Reels, 2FA
- 🔐 JWT + Cookie-based auth, Swagger API документация
- 📦 Загрузка изображений, короткие ссылки профилей

**Frontend** — TypeScript, React
- 🔗 [fakegram-frontend](https://github.com/IPodymov/fakegram-frontend)
- ✨ Современный интерфейс социальной сети

---

### 📊 Visualizer — Система визуализации данных
Приложение для визуализации и анализа данных с интерактивными графиками.

**Backend** — Python, FastAPI/Django
- 🔗 [visuliser-backend](https://github.com/IPodymov/visuliser-backend)
- ✨ API для обработки и визуализации данных

**Frontend** — Vue 3, TypeScript
- 🔗 [visualizer-front](https://github.com/IPodymov/visualizer-front)
- ✨ Интерактивные графики и дашборды

---

### 🏉 MGU Rugby API
REST API для управления информацией о регби-команде МГУ.

**Backend** — NestJS, TypeScript
- 🔗 [mgu-regby-api](https://github.com/IPodymov/mgu-regby-api)
- ✨ API для управления данными команды, расписанием и статистикой

---

## 📊 GitHub Статистика

<div align="center">
  
![GitHub stats](https://github-readme-stats.vercel.app/api?username=IPodymov&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=58A6FF&icon_color=1F6FEB&text_color=C9D1D9)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=IPodymov&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=58A6FF&text_color=C9D1D9)

</div>

---

## 📫 Связаться со мной

<div align="center">

💬 Открыт для обсуждения проектов, идей и возможностей сотрудничества!

**Telegram:** [t.me/ipodymov](https://t.me/ipodymov) | **VK:** [vk.com/soulgrok](https://vk.com/soulgrok) | **Email:** podymovv55@gmail.com

</div>

---

<div align="center">
  
### ✨ *"Хороший код — это код, который легко читать, легко поддерживать и приятно изменять"*

![Profile Views](https://komarev.com/ghpvc/?username=IPodymov&color=blue&style=for-the-badge)

**Спасибо за визит!** 🙌

</div>
